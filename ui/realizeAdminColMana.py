...
class AdminColManaWindow(QMainWindow):
    ...
    def showCollegeInform(self):
        ...
        # 右键菜单， 鼠标右键点击学院节点，弹出菜单，可以添加专业、删除学院
        # 鼠标右键点击专业节点，弹出菜单，可以删除专业、添加培养方案、删除培养方案、查看培养方案
    ...
    def showCollegeResultMenu(self):
        ...
        if self.ui.showCollegeResult.currentItem().parent() is None:
            # 添加专业
            addMajorAction = menu.addAction('添加专业')
            addMajorAction.triggered.connect(self.addMajor)
            # 删除学院
        ...
        # 选中的是专业
            ...
            # 添加培养方案
            addTrainProgramAction = menu.addAction('添加培养方案')
            addTrainProgramAction.triggered.connect(self.addTrainProgram)
            # 删除培养方案
            delTrainProgramAction = menu.addAction('删除培养方案')
            delTrainProgramAction.triggered.connect(self.delTrainProgram)
            # 查看培养方案
    ...
    def addMajor(self):
        ...
        # self.admin.addMajor(collegeID) # TODO: 连接数据库
    ...
    def addTrainProgram(self):
        ...
        collegeID = self.ui.showCollegeResult.currentItem().parent().text(0).split('[')[1].split(']')[0]
        majorName = self.ui.showCollegeResult.currentItem().text(0)
        # self.admin.addTrainProgram(collegeID, majorName) # TODO: 连接数据库
    ...
    def delTrainProgram(self):
        ...
        collegeID = self.ui.showCollegeResult.currentItem().parent().text(0).split('[')[1].split(']')[0]
        majorName = self.ui.showCollegeResult.currentItem().text(0)
        # self.admin.delTrainProgram(collegeID, majorName) # TODO: 连接数据库
